...
/**
 * IPL Report Page
 * Displays financial report for IPL (Iuran Pemeliharaan Lingkungan)
 * Shows income, expenses, balance, and category breakdown
 */
@Component({
  selector: 'app-ipl-report',
  standalone: true,
  imports: [
    CommonModule,
    IonicModule,
  ...
  templateUrl: './ipl-report.page.html',
  styleUrls: ['./ipl-report.page.scss']
})
export class IplReportPage implements OnInit, OnDestroy {
  ...
  private loadReport(): void {
    ...
    this.subscriptions.push(
      this.cashTransactionsService.getIplReport(startDate, endDate).subscribe({
        next: (data) => {
        ...
        error: (error) => {
          console.error('Error loading IPL report:', error);
          this.toastService.error('Gagal memuat laporan IPL');
          this.isLoading = false;
          this.loadingService.dismiss();
        }
      })
  ...
  export(): void {
    ...
    this.subscriptions.push(
      this.cashTransactionsService.exportIplReport(startDate, endDate).subscribe({
        next: (blob) => {
        ...
        error: (error) => {
          ...
          this.loadingService.dismiss();
          this.toastService.error('Gagal mengekspor laporan IPL');
        }
      })
    );
  }
