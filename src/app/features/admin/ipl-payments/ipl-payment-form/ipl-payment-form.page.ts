...
export class IplPaymentFormPage implements OnInit {
  ...
  constructor() {
    this.form = this.fb.group({
      ...
      kegiatanAmount: [20000, [Validators.min(0)]],  // Iuran kegiatan warga (optional, per-month) — default 20000
      kegiatanMonthCount: [1, [Validators.min(1), Validators.max(24)]],  // NEW: Number of months for kegiatan
      notes: ['']
    ...
    this.subscriptions.push(
      this.iplPeriodsService.getActive().subscribe({
        next: async (periods) => {
          ...
          // If the current month's period isn't available yet, auto-create it so the
          // user can still pay even before an admin generates periods.
          if (!this.findCurrentPeriod(this.periods)) {
            try {
              const ensured = await firstValueFrom(this.iplPeriodsService.ensureCurrent());
              if (ensured) {
                const refreshed = await firstValueFrom(this.iplPeriodsService.getActive());
                this.periods = refreshed.find((p) => p.id === ensured.id)
                  ? refreshed
                  : [ensured, ...refreshed];
              }
            } catch (e) {
              console.error('Error ensuring current period:', e);
            }
          }

          this.applyPeriodPrefill();

          // When nothing was prefilled, default the selection to the current period
          // (created above if it was missing) so the required-period validation passes
          // immediately and the user can pay right away. Override by picking another
          // period from the dropdown.
          if (!this.prefillPeriodId) {
            const current = this.findCurrentPeriod(this.periods);
            if (current) {
              this.form.get('periodId')?.setValue(current.id);
              this.onPeriodChange();
            }
          }
  ...
  /**
   * Find the period matching the current month/year within a list, or undefined.
   */
  private findCurrentPeriod(periods: IplPeriod[]): IplPeriod | undefined {
    const now = new Date();
    const month = now.getMonth() + 1;
    const year = now.getFullYear();
    return periods.find((p) => p.month === month && p.year === year);
  }
  ...
  getTotalAmount(): number {
    ...
    return Number(iplTotal) + Number(this.getKegiatanTotal());
  }

  /**
   * Get kegiatan amount (per-month) from form
   */
  ...
  /**
   * Whether iuran kegiatan is included in this payment.
   * Keys off amount > 0 so toggling to 0 excludes it entirely
   * (the DTO/service omit kegiatan when amount is falsy).
   */
  isKegiatanEnabled(): boolean {
    return this.getKegiatanAmount() > 0;
  }

  /**
   * Toggle iuran kegiatan on/off.
   * On  → nominal tetap Rp 20.000 & reset month count to 1.
   * Off → set amount to 0 so it is excluded from the payment.
   */
  toggleKegiatan(event: CustomEvent): void {
    const enabled = Boolean(event.detail?.checked);
    if (enabled) {
      this.form.get('kegiatanAmount')?.setValue(20000);
      this.form.get('kegiatanMonthCount')?.setValue(1);
    } else {
      this.form.get('kegiatanAmount')?.setValue(0);
    }
  }

  /**
   * Get total kegiatan amount = per-month amount × month count
   */
  getKegiatanTotal(): number {
    return this.getKegiatanAmount() * this.getKegiatanMonthCount();
  }

  /**
   * Get kegiatan month count from form
   */
  getKegiatanMonthCount(): number {
    const value = this.form.get('kegiatanMonthCount')?.value;
    return value && value > 0 ? Number(value) : 1;
  }

  /**
   * Check if kegiatan is a multi-month payment
   */
  isKegiatanMultiMonth(): boolean {
    return this.getKegiatanMonthCount() > 1 && this.getKegiatanAmount() > 0;
  }

  /**
   * Increment kegiatan month count
   */
  incrementKegiatanMonthCount(): void {
    const current = this.getKegiatanMonthCount();
    if (current < 24) {
      this.form.get('kegiatanMonthCount')?.setValue(current + 1);
    }
  }

  /**
   * Decrement kegiatan month count
   */
  decrementKegiatanMonthCount(): void {
    const current = this.getKegiatanMonthCount();
    if (current > 1) {
      this.form.get('kegiatanMonthCount')?.setValue(current - 1);
    }
  ...
  async onSubmit(): Promise<void> {
    ...
    const dto: CreateIplPaymentDto = {
      ...
      kegiatanMonthCount: formValue.kegiatanAmount
        ? (formValue.kegiatanMonthCount || 1)
        : undefined,  // NEW: only sent when kegiatanAmount is provided
  ...
  private async showSuccessModal(payment: IplPayment): Promise<void> {
    ...
    if (payment.kegiatanPayment) {
      const kegMonthCount = payment._meta?.kegiatanMonthCount || payment.kegiatanPayment.monthCount || 1;
      rows.push({
        label: 'Iuran Kegiatan',
        value: kegMonthCount > 1
          ? `${this.formatCurrency(payment.kegiatanPayment.amount)} (${kegMonthCount} bln)`
          : this.formatCurrency(payment.kegiatanPayment.amount),
      });
    }
